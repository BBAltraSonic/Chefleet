...
-   Sub-tasks
    ...
    -   Create Supabase projects for dev & staging & prod.
        
    -   Enable Auth (phone OTP + email) and turn on Row Level Security.
        
    -   Create Storage bucket `vendor_media` and default access policy (signed URLs by default).
        
    -   Configure Realtime (allowing channels but RLS still enforced).
...
-   Sub-tasks
    ...
    -   Add full table set: `users_public`, `user_addresses`, `vendors`, `dishes`, `orders`, `order_items`, `messages`, `favourites`, `device_tokens`, `notifications`, `payments` (future), `audit_logs`, `moderation_reports`, `app_settings`.
        
    -   Add enums and indexes (price cents, vendor geography point with PostGIS if available).
        
    -   Add unique and FK constraints, idempotency_key for orders.
        
    -   Commit SQL migrations to `chefleet-infra`.
...
-   Sub-tasks
    ...
    -   Draft RLS policies for every table (read/write rules by role).
        
    -   Ensure `orders` SELECT/UPDATE only for `buyer_id` or `vendor_id` or admin.
        
    -   Enforce message insertion only when the sender is party to the order.
        
    -   Disallow direct client-side status updates — status changes must be performed via Edge functions.
...
-   Sub-tasks
    ...
    -   Define RPCs/Edge endpoints: `create_order`, `change_order_status`, `generate_pickup_code`, `send_push`, `upload_image_signed_url`, `report_user`, `process_payment_webhook`.
        
    -   Document contracts for each endpoint (input/response/errors).
        
    -   Decide auth model for Edge functions (Supabase JWT verification).
...
-   Sub-tasks
    ...
    -   Export color tokens, typography, spacing, components (buttons, cards, nav, FAB, dialogs).
        
    -   Provide ThemeData and component examples for Flutter (liquid glass nav background, FAB notch).
        
-   Owner: Frontend Lead
    
-   Deliverable: `design-system` folder with ThemeData and component references.
...
-   Sub-tasks
    ...
    -   Scaffold Flutter app with folder structure: `auth`, `home`, `map`, `feed`, `dish`, `order`, `chat`, `profile`, `settings`.
        
    -   Implement bottom navigation with liquid glass style and center notch + FAB.
        
    -   Implement persistent Map widget instance across tabs.
        
-   Owner: Frontend
    
-   Deliverable: Working scaffold branch.
...
-   Sub-tasks
    ...
    -   Implement phone OTP flow with Supabase Auth.
        
    -   On first sign in, complete profile (name, avatar, default address).
        
    -   Store minimal metadata (notif prefs) in `users_public.metadata`.
        
-   Owner: Frontend / Backend
    
-   Deliverable: Auth flow integrated; user rows created in DB.
...
-   Sub-tasks
    ...
    -   Integrate Google Maps SDK (Android & iOS) + Routes API key config.
...
-   Sub-tasks
    ...
    -   Edge function validates dish availability, calculates total (server-side), generates pickup_code, inserts `orders` + `order_items`, notifies vendor via realtime and push.
